...
- **Map legibility** *(every board)* — route lines are ranked rather than all drawn alike: rapid transit reads as the spine, commuter rail and bus routes recede behind it, and each Metro line gets a dark casing so six bright routes stay separable where they braid through a city centre. Vehicle name chips are spaced properly and capped to what the map can hold — scaled to its area, so the same board is calm on a tablet and not austere on a 1080p wall display. Every vehicle keeps its marker; only the surplus chips are dropped.
- **🚇 Mode glyphs on the map** *(every board)* — vehicles are drawn as what they *are*: a train, a tram, a bus, a ferry. Colour still carries the line, so the two facts stop competing for one channel — a red train and a red shuttle bus on the same corridor can no longer be confused. Buses are a **dark blue** on every board, two steps below the light blues the rail lines use. They are the one mode with no line colour to carry, since no agency here brands its bus network, so what they used to be drawn in was picked arbitrarily and picked badly: a light blue sitting beside the European boards' own metro blue and WMATA's Blue Line, and a SEPTA amber wedged between the B line's orange and the G's yellow. Boston keeps its own — the Silver Line really is branded silver. Switch back to the original dots with **Vehicles on the map → Dots** in ⚙︎.
- **🔤 SEPTA Metro markers** *(Philadelphia board)* — SEPTA's 2024 wayfinding standard names things PROVIDER → SYSTEM → LINE → SERVICE PATTERN, and the board already speaks the bottom level: its bundled subway timetable is keyed `L1`/`B1`–`B3`, and SEPTA's own live feed publishes `T1`–`T5`, `M1`, `G1`, `D1`/`D2`. So each vehicle's **line** is the first character, and the map draws the line's own letter tile — the orange **B**, the blue **L** — which is what the pylon, the platform sign and the pocket map all show. ⚙︎ → **SEPTA Metro markers → Vehicles** swaps them for mode pictograms (trolley, metro car, elevated, mainline rail) instead. Regional Rail stays out of the letter scheme deliberately: it is a separate system in SEPTA's own hierarchy and its lines are *named*, not lettered. The digit in a code is required, because SEPTA also runs **bus** routes lettered G, H, J, K, L and R — a bare `L` is the L bus, not the Market–Frankford Line. All six lines are drawn: L and B from the subway, and the **T** trolleys, **G** on Girard, **D** to Media and Sharon Hill and the **M** up to Norristown from SEPTA's live vehicle feed, which carries them alongside the bus network. They ride the SEPTA Metro card's own map layer, so switching *SEPTA Bus* off in ⚙︎ no longer takes the trolleys with it. All six are also **drawn on the map as lines**, traced from SEPTA's own GTFS geometry by `gen-septa-metro-shapes.py` into a 7.5 KB `septa-metro-shapes.json` — real track that follows Market Street rather than the dogleg you get from joining stops with straight segments, and it costs a fraction of the ~2 MB that adding those four to the timetable bundle would. One shape per **service pattern**, not per line: `T1`–`T5` are five physically different trolley routes that share a tunnel through Center City and then fan out to five termini, and `D1`/`D2` split at Drexel Hill for Media and Sharon Hill. Drawing one line per letter put one branch on the map and silently dropped the rest. Patterns that really do share track (`B1` local, `B2` express) draw over each other in the same colour and look like the one line they are. Departure **times** now cover all thirteen patterns too: `gen-septa-subway-schedule.py`'s `ROUTES` was widened from the two rapid-transit spines to the whole Metro set, taking the bundle from 0.95 MB to 3.4 MB — 0.62 MB gzipped, which is what actually crosses the wire. The card picks the nearest stop **per line** rather than simply the nearest stop, because trolley stops come every couple of blocks and 711 stations would otherwise hand the whole card to whichever trolley runs past the door and push the L and B off it.
- **🟢 Your commute** *(every board)* — save the stop you leave from and how long you take to walk there, and the board counts down to when **you** have to move, not when the train arrives: *"3 mins to leave"*, amber at two minutes, and an honest *"you have missed this one"* rather than silently skipping to the next. Filters the departures the board already has, so there is no router and no server. Set it up from the card, or hide it in ⚙︎ → Show on board. Out-of-service trains (WMATA publishes "No Passenger" moves in the same list) are excluded — a countdown to a train you cannot board is worse than none.
- **♿ Lifts & escalators** *(Bethesda board)* — WMATA's elevator/escalator outage feed, which answers the one question the departure boards cannot: whether you can actually get down to the platform. Outages at *your* stations come first and lifts always outrank escalators; when nothing is wrong at your stations it says so ("step-free at all 3") rather than vanishing, because a card that disappears on good news can't be told from one that failed to load.
- **📊 Track record** *(every board with a real-time feed)* — how long you actually wait, today versus your own last two weeks, with a seven-day bar per stop. Deliberately **not** an on-time percentage: Metrorail publishes no timetable, so there is no such thing as a "late" Metro train in the data and any percentage would be invented. Median wait *is* measurable and moves with headway, which is the thing that goes wrong. Recorded in your browser only — nothing is uploaded. On a board whose feeds are **all** bundled timetables (Los Angeles, New Jersey) the card removes itself: a "typical wait" measured off a timetable is just the printed headway read back, and it could never show the deterioration the card exists to show.
- **☂ Rain radar** *(every board, off by default, in ⚙︎)* — live precipitation from [RainViewer](https://www.rainviewer.com) under the vehicles on the map, plus a chip on the map reading "raining here now" or "rain in ~20 min" — sampled at **your** location, not the city centroid, so it is about the walk to your stop. No key, no account, worldwide, so it works the same on the European boards.
...
- **Bundled timetables revalidate** — the `*-schedule.json` bundles are loaded with `cache: "no-cache"`, so an unchanged one 304s and costs nothing but a **redeployed** one is picked up without a hard refresh. Without it a long-running kiosk holds whatever it first downloaded: a board was still serving a timetable from two weeks earlier, whose service calendar had since run out, and so drew empty departure cards and raised the "timetable out of date" warning about a file that had actually been refreshed in the repo days before. The bundle was fine; the copy in the browser was not.
...
| Map tiles | [Esri](https://services.arcgisonline.com) Dark/Light Gray Canvas (Esri, HERE, Garmin, OpenStreetMap) | ❌ none | Free |
